...
while(True):
    ...
    if exist:
        ...
        parser.add_argument('--input', type=str, default="data/1.png", help='input image')
        ...
        b[:,1:] = np.array(faces)
        write_ply(filename=opt.input , points=pd.DataFrame(torch.cat((pointsReconstructed.cpu().data.squeeze(), grid_pytorch), 1).numpy()), as_text=True, text=True, faces = pd.DataFrame(b.astype(int)))
